...
RightSecVisual3_WithinFear <- c(10.0222, 11.7093, 21.1406, -6.0738, -0.8584, 7.8678, 
                                2.2344, -0.2560, -1.4060, 6.6150, 3.8500, 1.2002, 
                                -2.1724, -8.6665, 3.8655, 3.4370, 2.8379)
...
roi_data_vsN <- list(
  RightSecVisual2_CrossHappy        = RightSecVisual2_CrossHappy,
  LeftSecVisual1_WithinHappy        = LeftSecVisual1_WithinHappy,
  RightSecVisual1_WithinHappy       = RightSecVisual1_WithinHappy,
  RightFFA_WithinHappy              = RightFFA_WithinHappy,
  LeftVisualAssoc1_WithinHappy      = LeftVisualAssoc1_WithinHappy,
  LeftSecVisual2_WithinHappy        = LeftSecVisual2_WithinHappy,
  RightAntPFC_CrossFear             = RightAntPFC_CrossFear,
  Presuppmot_CrossFear              = Presuppmot_CrossFear,
  LeftVisualAssoc2_CrossFear        = LeftVisualAssoc2_CrossFear,
  LeftSupramGy_CrossFear            = LeftSupramGy_CrossFear,
  LeftFFA1_CrossFear                = LeftFFA1_CrossFear,
  LeftFFA2_WithinFear               = LeftFFA2_WithinFear,
  RightSecVisual3_WithinFear        = RightSecVisual3_WithinFear,
  LeftSecVisual3_WithinFear         = LeftSecVisual3_WithinFear,
  LeftSecVisual4_WithinFear         = LeftSecVisual4_WithinFear
)

# Run the frequency t-test
results_freq_vsN <- data.frame(
  ROI = names(roi_data_vsN),
  Mean = sapply(roi_data_vsN, mean),
  SD = sapply(roi_data_vsN, sd),
  N = sapply(roi_data_vsN, length),
  t = sapply(roi_data_vsN, function(x) t.test(x, mu = 0)$statistic),
  df = sapply(roi_data_vsN, function(x) t.test(x, mu = 0)$parameter),
  p = sapply(roi_data_vsN, function(x) t.test(x, mu = 0)$p.value),
  Cohen_d = sapply(roi_data_vsN, function(x) (mean(x) - 0) / sd(x))
)

# Correct for multiple comparisons
results_freq_vsN$p_Bonf <- p.adjust(results_freq_vsN$p, method = "bonferroni")
